...
func (r *SubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	log.Info("Reconciling subscription " + req.Name)

	// Get subscription
	err := r.Get(ctx, req.NamespacedName, subscription)
	if err != nil {
	...
	// Subscription is marked for deletion
	if subscription.DeletionTimestamp != nil {
		log.Info("Deletion timestamp found for subscription " + req.Name)
		return ctrl.Result{}, nil
	}

	// Handle subscription
	return r.handleSubscription(ctx, log, subscription)
}
